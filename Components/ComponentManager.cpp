...
void ComponentManager::DebugPrint()
{
...
void ComponentManager::Initialize(unsigned long maxSize)
{
...
//RigidBody* ComponentManager::CreateRigidBody()
//{
...
//void ComponentManager::DestroyRigidBody(unsigned int index)
//{
...
void ComponentManager::CleanupDeletions()
{
  /*  for (int i = 0; i < mMaxSize; ++i)
    {
        RigidBody* data = reinterpret_cast <RigidBody*>(mRigidBodyArray);
...
void ComponentManager::UpdateSubsystem(size_t timeDelta)
{
    //RigidBody* data = reinterpret_cast <RigidBody*>(mRigidBodyArray);
